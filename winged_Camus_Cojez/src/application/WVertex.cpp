...
void WVertex::computeNormal() {
    ...
    do {
        ...
        if(this == e->begin() && e->left() != nullptr) {
            average += e->left()->normal();
            e = e->predLeft();
            cpt++;
        }
        else if(this == e->end() && e->right() != nullptr) {
    ...
    //q5, repart dans l'autre sens si nous ne sommes pas retournés au point de départ
    //pas totalement bon (qu'une seule jambe ??)
    if(e != start) {
        e = start;
        do {

            if(this == e->end() && e->left() != nullptr) {
                average += e->left()->normal();
                e = e->predLeft();
                cpt++;
            }
            else if(this == e->begin() && e->right() != nullptr) {
                average += e->right()->normal();
                e = e->predRight();
                cpt++;
            }
